...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* Header */
  header{display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:3px solid var(--accent);background:linear-gradient(90deg,#070707,#0b0b0b)}
  .brand{display:flex;gap:12px;align-items:center}
  .logo{width:44px;height:44px;background:linear-gradient(135deg,#e00,#900);border-radius:8px;display:flex;align-items:center;justify-content:center;font-weight:800;color:#fff;font-size:20px}
  h1{margin:0;font-size:18px;color:var(--accent);letter-spacing:0.6px}
  nav{display:flex;gap:10px;align-items:center}
  nav button{background:transparent;color:#fff;border:1px solid rgba(255,0,0,0.08);padding:8px 10px;border-radius:8px;cursor:pointer;font-weight:600}
  nav button:hover{background:rgba(255,0,0,0.06)}
  ...
  .slide{flex:0 0 100%;padding:34px;display:flex;gap:20px;align-items:center}
  .slide .left{flex:1}
  .slide .right{flex:1;display:flex;align-items:center;justify-content:center}
  .slide h2{color:var(--accent);margin:0 0 10px;font-size:28px}
  ...
  @media (max-width:980px){.grid{grid-template-columns:1fr}.hero{height:300px}}
  ...
  .modal-box{width:380px;background:linear-gradient(180deg,#0b0b0b,#090909);padding:18px;border-radius:12px;border:1px solid #161616}
...
<header>
  ...
  <nav>
    ...
    <button onclick="openModal('login')">Login / Signup</button>
  </nav>
</header>
  ...
  <div id="home" class="fade-in">
    <div class="hero card">
      <div class="slides" id="slides">
        <div class="slide">
          <div class="left">
            ...
            <p class="muted">Accurate signals and fast payments. M-Pesa Till: <strong style="color:var(--accent)">3675058</strong></p>
          ...
          <div class="right"><img src="https://images.unsplash.com/photo-1559523160-1a5c3f6f0a12?q=80&w=800&auto=format&fit=crop&ixlib=rb-4.0.3&s=11c7f6eed9b3d0db4a8cc3aca58658bf" alt="" style="max-width:100%;border-radius:8px;box-shadow:0 12px 40px rgba(0,0,0,0.6)"></div>
        </div>
        ...
        <div class="slide">
          ...
          <div class="right"><img src="https://images.unsplash.com/photo-1531297484001-80022131f5a1?q=80&w=800&auto=format&fit=crop&ixlib=rb-4.0.3&s=9f23a2cd2b3d7a3172a6b9a4b2b3e6fc" style="max-width:100%;border-radius:8px"></div>
        </div>

        <div class="slide">
          <div class="left">
            <h2>Fast Payments (M-Pesa)</h2>
            <p class="muted">Deposit instantly using M-Pesa Till 3675058. Send the transaction ID via WhatsApp for quick verification.</p>
            <div style="margin-top:12px"><button class="btn" onclick="payPrompt()">Pay Now</button></div>
          </div>
          <div class="right"><img src="https://images.unsplash.com/photo-1532634896-26909d0d3d57?q=80&w=800&auto=format&fit=crop&ixlib=rb-4.0.3&s=0f7f2d1b4b2e2a5e5c3a1a1d1f0b2c7b" style="max-width:100%;border-radius:8px"></div>
        </div>
      </div>
      ...
      <div class="slider-controls">
        ...
        <div>
          <button class="chip" onclick="prevSlide()">Prev</button>
          <button class="chip" onclick="nextSlide()">Next</button>
        </div>
      </div>
    </div>
      ...
      <div class="right-col">
        <div class="card">
          ...
          <div style="margin-top:12px" class="chart-wrap card">
            <canvas id="priceChart" style="max-height:260px"></canvas>
          </div>
...
<div id="modal" class="modal">
  <div class="modal-box fade-in">
    ...
    <div id="signupBox" style="display:none">
      ...
      <div style="margin-bottom:10px">
        <input id="signupPhone" placeholder="Phone (2547...)" />
      </div>
      <div style="margin-bottom:12px">
...
<footer>
  ...
  <div class="socials">
    <a href="https://wa.me/254796518099" target="_blank" class="chip">WhatsApp</a>
...
for(let i=0;i<total;i++){
  const d = document.createElement('div'); d.className='dot' + (i===0?' active':''); d.onclick = ()=>goTo(i); dots.appendChild(d);
}
...
function tickPrices(){
  basePrices = basePrices.map((p,i)=>randomWalk(p, i===2?0.008:0.003)); // JPY more volatile
...
setInterval(()=>{ // push new simulated point
  const t = new Date().toLocaleTimeString().slice(0,5);
  const last = parseFloat(chart.data.datasets[0].data.slice(-1)[0]);
  const next = +(last * (1 + (Math.random()-0.5)*0.002)).toFixed(4);
  chart.data.labels.push(t); chart.data.datasets[0].data.push(next);
...
function openModal(mode='login'){ modal.classList.add('show'); toggleTab(mode); }
function closeModal(){ modal.classList.remove('show'); document.getElementById('loginMsg').innerText=''; document.getElementById('signupMsg').innerText=''; }
...
/* ---------- Payment prompt (front-end only) ---------- */
function payPrompt(){
  alert('To pay: use M-Pesa Till 3675058\nAfter payment send transaction ID via WhatsApp to +254796518099');
}

/* ---------- Quick helpers ---------- */
function openPricing(){ alert('Pricing plans: Starter, Pro, VIP — contact us for details.'); }
